...
1. 저장된 두 계정의 인증 토큰이 로그, repo, crash report, UI 오류에 노출되지 않는다.
...
| 정상 종료 시간 초과 | 아니오 | 강제 종료 없이 차단 | 사용자가 직접 판단 |
