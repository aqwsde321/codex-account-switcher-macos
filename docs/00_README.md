...
3. 완료: custom async harness의 fake fixture 89개 테스트와 `inspect`/`profiles list`/`profile capture`/`profile sync-active`/`switch`/`recovery status`/`recovery restore`
...
11. 다음: 메뉴바 활성 인증 동기화와 상세 단계·수동 복구·재로그인 동작 연결
...
`08_implementation_handoff.md` Step 9의 동기화와 상세 상태·복구 UI를 구현해.
