...
- 어느 계정에서든 `resetsAt` 변경을 감지하면 전체 계정 한도를 다시 조회한다. 5시간 창의 남은 사용량이 `100%`인 계정의 `⚡`을 계정당 한 번씩 순차 실행하고, 실행 후 한도와 새 `resetsAt`을 다시 반영한다. 상태에는 감지된 계정·창과 이전·새 `resetsAt`을 표시한다.
...
- 토큰 사용용 작업 영역: `~/Library/Application Support/CodexAccountSwitcher/token-use-home/`
